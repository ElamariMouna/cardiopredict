...
sel = st.sidebar.selectbox("Profil:", profile_names)
p = PROFILES[sel]
st.sidebar.caption(f"📝 {p['desc']}")

# Bouton pour charger le profil
if st.sidebar.button("📥 Charger ce profil", use_container_width=True):
    for k, v in p.items():
        if k != 'desc':
            st.session_state[f's_{k}'] = v

# Initialize session state with default profile values
for k, v in p.items():
    if k != 'desc' and f's_{k}' not in st.session_state:
        st.session_state[f's_{k}'] = v
...
gender = st.sidebar.selectbox("Sexe", ["Homme", "Femme"], 
    index=0 if st.session_state.get('s_gender', p['gender'])==1 else 1)
age = st.sidebar.slider("Âge", 18, 100, int(st.session_state.get('s_age', p['age'])))
weight = st.sidebar.slider("Poids (kg)", 30, 200, int(st.session_state.get('s_weight', p['weight'])))

st.sidebar.markdown("### ❤️ Diagnostics")
mi = st.sidebar.checkbox("Infarctus", value=bool(st.session_state.get('s_myocardial_infarction', p['myocardial_infarction'])))
hf = st.sidebar.checkbox("Insuffisance cardiaque", value=bool(st.session_state.get('s_heart_failure', p['heart_failure'])))
arr = st.sidebar.checkbox("Arythmie", value=bool(st.session_state.get('s_arrhythmia', p['arrhythmia'])))
vd = st.sidebar.checkbox("Valvulopathie", value=bool(st.session_state.get('s_valvular_disease', p['valvular_disease'])))
cad = st.sidebar.checkbox("Coronaropathie", value=bool(st.session_state.get('s_coronary_artery_disease', p['coronary_artery_disease'])))
diag_arr = st.sidebar.checkbox("Diag: Arythmie", value=bool(st.session_state.get('s_diag_Arrhythmia', p['diag_Arrhythmia'])))
diag_hf = st.sidebar.checkbox("Diag: IC", value=bool(st.session_state.get('s_diag_Heart_Failure', p['diag_Heart_Failure'])))
diag_mi = st.sidebar.checkbox("Diag: IDM", value=bool(st.session_state.get('s_diag_Myocardial_Infarction', p['diag_Myocardial_Infarction'])))

st.sidebar.markdown("### 🩸 Biomarqueurs")
trop = st.sidebar.number_input("Troponine T (ng/mL)", 0.0, 50.0, float(st.session_state.get('s_troponin_t', p['troponin_t'])), 0.01)
ckmb = st.sidebar.number_input("CK-MB (U/L)", 0.0, 500.0, float(st.session_state.get('s_ck_mb', p['ck_mb'])), 1.0)
lactate_max = st.sidebar.number_input("Lactate Max (mmol/L)", 0.0, 30.0, float(st.session_state.get('s_lactate_max', p['lactate_max'])), 0.1)
lactate_min = st.sidebar.number_input("Lactate Min (mmol/L)", 0.0, 30.0, float(st.session_state.get('s_lactate_min', p['lactate_min'])), 0.1)
creat_max = st.sidebar.number_input("Créatinine Max (mg/dL)", 0.0, 20.0, float(st.session_state.get('s_creatinine_max', p['creatinine_max'])), 0.1)
creat_min = st.sidebar.number_input("Créatinine Min (mg/dL)", 0.0, 20.0, float(st.session_state.get('s_creatinine_min', p['creatinine_min'])), 0.1)
bun_max = st.sidebar.number_input("BUN Max (mg/dL)", 0.0, 200.0, float(st.session_state.get('s_bun_max', p['bun_max'])), 1.0)
bun_min = st.sidebar.number_input("BUN Min (mg/dL)", 0.0, 200.0, float(st.session_state.get('s_bun_min', p['bun_min'])), 1.0)

st.sidebar.markdown("### 💉 Hémodynamique")
sbp_min = st.sidebar.number_input("SBP Min (mmHg)", 30, 250, int(st.session_state.get('s_sbp_min', p['sbp_min'])))
sbp_max = st.sidebar.number_input("SBP Max (mmHg)", 30, 300, int(st.session_state.get('s_sbp_max', p['sbp_max'])))
dbp_min = st.sidebar.number_input("DBP Min (mmHg)", 20, 150, int(st.session_state.get('s_dbp_min', p['dbp_min'])))
dbp_max = st.sidebar.number_input("DBP Max (mmHg)", 20, 200, int(st.session_state.get('s_dbp_max', p['dbp_max'])))
map_min = st.sidebar.number_input("MAP Min (mmHg)", 20, 150, int(st.session_state.get('s_map_min', p['map_min'])))
map_max = st.sidebar.number_input("MAP Max (mmHg)", 20, 200, int(st.session_state.get('s_map_max', p['map_max'])))
hr_min = st.sidebar.number_input("FC Min (bpm)", 20, 200, int(st.session_state.get('s_hr_min', p['hr_min'])))
hr_max = st.sidebar.number_input("FC Max (bpm)", 20, 250, int(st.session_state.get('s_hr_max', p['hr_max'])))

st.sidebar.markdown("### 🫁 Respiratoire")
resp_min = st.sidebar.number_input("FR Min (/min)", 5, 50, int(st.session_state.get('s_resp_rate_min', p['resp_rate_min'])))
resp_max = st.sidebar.number_input("FR Max (/min)", 5, 60, int(st.session_state.get('s_resp_rate_max', p['resp_rate_max'])))
spo2_min = st.sidebar.number_input("SpO2 Min (%)", 50, 100, int(st.session_state.get('s_spo2_min', p['spo2_min'])))
spo2_max = st.sidebar.number_input("SpO2 Max (%)", 50, 100, int(st.session_state.get('s_spo2_max', p['spo2_max'])))
temp_min = st.sidebar.number_input("Temp Min (°C)", 32.0, 42.0, float(st.session_state.get('s_temperature_min', p['temperature_min'])), 0.1)
temp_max = st.sidebar.number_input("Temp Max (°C)", 32.0, 42.0, float(st.session_state.get('s_temperature_max', p['temperature_max'])), 0.1)
gluc_min = st.sidebar.number_input("Glucose Min (mg/dL)", 20, 500, int(st.session_state.get('s_glucose_min', p['glucose_min'])))
gluc_max = st.sidebar.number_input("Glucose Max (mg/dL)", 20, 800, int(st.session_state.get('s_glucose_max', p['glucose_max'])))

st.sidebar.markdown("### 💊 Support Thérapeutique")
norepi = st.sidebar.checkbox("Norépinéphrine", value=bool(st.session_state.get('s_norepinephrine', p['norepinephrine'])))
epi = st.sidebar.checkbox("Épinéphrine", value=bool(st.session_state.get('s_epinephrine', p['epinephrine'])))
dopa = st.sidebar.checkbox("Dopamine", value=bool(st.session_state.get('s_dopamine', p['dopamine'])))
dobu = st.sidebar.checkbox("Dobutamine", value=bool(st.session_state.get('s_dobutamine', p['dobutamine'])))
vaso_count = st.sidebar.number_input("Nb vasopresseurs", 0, 5, int(st.session_state.get('s_vasopressor_count', p['vasopressor_count'])))
mech_vent = st.sidebar.checkbox("Ventilation mécanique", value=bool(st.session_state.get('s_mechanical_ventilation', p['mechanical_ventilation'])))
